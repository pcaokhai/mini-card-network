...
| acquirer | `tran_log.approved_amount`, `balance_amount`, `balance_currency`, `original_rrn`, `trace_id`, `reversal_reason` (migration 00007) | Transaction detail, W3C trace id, the 0420's DE 39 reason for `reversalReason` | POS-G8, JRN-G3, JRN-G7 |
| acquirer | `idempotency_record.rrn`, `tran_log.completed_by` (migration 00008) | The RRN a reserved key sent, so a retry answers from `tran_log`; the completion that consumed a pre-auth | #117 review S1, S2 |
