...
| SEC-G15 | The issuer never uses a rotated ZAK/ZPK: `VerifySecurity` and `Respond` read `ZAK_HEX` from env once, and only the retired-key fallback reads `key_store`. A ZAK rotation followed by a gateway restart would make every MAC fail (RC 96). The gateway therefore answers `POST …/rotations {keyType: ZAK}` with 409 `conflict` (#121). | `issuer-jpos` MAC participants; `internal/api/rotations.go` guard | ISS | The issuer loads its active ZAK/ZPK from `key_store`, per message or on activation; then lift the gateway's ZAK guard |
| SEC-G16 | A key-change 0800/161 that gets no 0810 after every attempt leaves the rotation `FAILED` with an unknown outcome. The issuer may or may not have activated the key, so the gateway keeps it PENDING and retries a response MAC under it (#121); nothing ever settles it. | `internal/rotation/runner.go` `runSend0800161`; `store.MACFallbackKeys` | GW + ISS + contracts | A key-status query (e.g. 0800 with a KCV check) to resolve the outcome, then activate or retire the PENDING key |
