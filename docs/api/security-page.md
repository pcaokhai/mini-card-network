...
| SEC-G7 | ~~Gateway problems use bare slugs (`type` = `title` = `"get-rotation-failed"`), not `https://mcn.local/problems/…` URIs. They have no `instance` or `traceId`, and `detail` leaks internal error text (pgx and strconv messages). None of the slugs is in docs/04 §3.~~ | `lab.go:73-77` `problem()`; live responses above | GW | **Fixed** (#PRN, P-1): URI types from the docs/04 §3 list, `instance`, `traceId`; generic `detail` for 500s |
...
| 1.9 | 2026-09-26 | SEC-G7 fixed (#PRN, P-1) |
