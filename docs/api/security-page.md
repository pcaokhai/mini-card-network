...
| SEC-G7 | ~~Gateway problems use bare slugs (`type` = `title` = `"get-rotation-failed"`), not `https://mcn.local/problems/…` URIs. They have no `instance` or `traceId`, and `detail` leaks internal error text (pgx and strconv messages). None of the slugs is in docs/04 §3.~~ | `lab.go:73-77` `problem()`; live responses above | GW | **Fixed** (#131, P-1): URI types from the docs/04 §3 list, `instance`, `traceId`; generic `detail` for 500s |
...
| SEC-G10 | The gateway loads the ZAK once at startup. After a ZAK rotation it keeps computing outbound MACs with the retired ZAK, which the issuer accepts for 5 minutes only, then answers RC 96. The UI doesn't offer ZAK, but the API accepts it. | `main.go` `loadActiveZAK`; `purchase/service.go` field `zak` used in `ComputeMAC` | GW | **Fixed** in #127: purchase, advtxn, `purchase.MACVerifier` and the SAF worker take an `hsm.ZAKSource` (`rotation.ActiveKeys`, reloaded after ACTIVATE) and read it per message, so a rotated ZAK MACs and verifies from the next message without a restart. The 409 guard on ZAK rotations is lifted: the issuer reads its ACTIVE ZAK live since #122 (SEC-G15) |
...
| SEC-G23 | A key change's activation and its `key_change.activated` audit row committed separately: `activate()` committed, then the audit insert ran on another connection. If that insert failed, `ReceiveKeyChange` answered 0810 RC 96 while the new key was already ACTIVE, so the acquirer believed the change had failed. The issuer also answers 96 for a genuine rejection (an old key replayed, SEC-G18), so a gateway alone couldn't tell the two apart | `ReceiveKeyChange.receive`; `KeyStoreRepository.activate` | ISS | **Fixed** in #128: `KeyStoreRepository.activate(id, alsoInThisTransaction)` runs the audit insert on the activation's own connection before the commit. A failed audit write rolls the activation back, and the answer (96) matches `key_store`: nothing changed. On success the key is ACTIVE and audited in one commit. The SF2 path (a concurrent resend activated the same key) still answers 00. Gateway side mitigated in #127 as defence in depth: an 0810 with RC 96 is treated as an unknown outcome (resent with the same cryptogram up to `ROTATION_SEND_ATTEMPTS`, then FAILED with the key kept PENDING), never a decline that retires the key. Open: the issuer should answer a distinct decline RC for a rejected replayed key, so the gateway isn't left resending a key it will never accept |
...
| 1.8 | 2026-09-26 | SEC-G10 fixed (#127): the gateway reads the ZAK per message; ZAK rotation re-enabled; SEC-G23 gateway-side mitigation (0810 RC 96 treated as outcome-unknown) noted. |
| 1.9 | 2026-09-26 | SEC-G7 fixed (#131, P-1) |
