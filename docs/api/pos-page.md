...
- **Provider rules.**
  ...
  4. As a purchase (§4.1 rules 5–7): a link that is down records DECLINED RC `91`; any send failure after the write queues a reason-`68` 0420 and returns `REVERSAL_PENDING`; a bad incoming MAC (checked against the 0110) is DECLINED RC `96` with a reason-`06` 0420. The same holds for every advanced flow (completion 0230, refund and balance inquiry 0210).
...
| POS-G4 | ~~Advanced transactions break "unknown outcome ⇒ reversal" (root CLAUDE.md §6.4)~~ | **Fixed** (#114): every send failure after the write marks the row TIMED_OUT and queues a 0420 (DE 39 68) through the SAF, for purchases and all four advanced flows; link down, or a link that drops before the write, records DECLINED RC 91 (`purchase.SendFailureStatus`) | GW | Done |
...
| 1.1 | 2026-09-25 | POS-G4, POS-G5 and POS-G9 fixed (#114) |
