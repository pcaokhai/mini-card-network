...
| POS-G8 | ~~POST responses miss fields the schema defines~~ | **Fixed** (#117): POST responses carry `responseLabel`, `latencyMs` and the request span's `traceId`, stored in `tran_log.trace_id` (migration 00007) and returned by the GET detail | GW | Done |
