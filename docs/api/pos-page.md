...
| POS-G16 | ~~No sweeper for orphan SENT / TIMED_OUT rows~~ | **Fixed** (#123): `saf.Sweeper`, run by main every 30 s, finds rows sent more than 90 s ago that are still SENT, TIMED_OUT with nothing in saf_queue, or DECLINED RC 96 (a bad MAC) with nothing queued, for all but balance inquiries (`TranLogRepository.FindOrphans`). A MAC-failure orphan gets a reason-06 0420. A completion without an original RRN is logged, not repeated. The post-send status write is guarded on SENT (`UpdateStatusFrom`), so a stalled request's late answer never overwrites a swept row; that request is answered from the row. Since #117 a request is marked SENT only as the last step before the send, so a pre-send failure is never swept. It moves a SENT row to TIMED_OUT (guarded on SENT), then queues a reason-68 0420, or for a completion its 0220 rebuilt from the row (same STAN and DE 7) for 0221 repeats; a balance inquiry gets nothing. This closes the post-commit windows N1 and N2 of the #114 review. Was: If the gateway dies between the send and recording the outcome, the row stays SENT (or TIMED_OUT with nothing queued) forever; nothing reverses it on restart. The same short windows exist after commit: between the send and the status update (#114 review N1), and between the status update and the SAF insert (N2); a crash in either leaves the row unreversed | GW | Done |
...
| 1.5 | 2026-09-26 | POS-G16 fixed (#123): orphan sweeper |
