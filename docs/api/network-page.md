...
| --- | --- | --- | --- | --- |
| NET-G1 | A manual echo doesn't move `lastEchoAt`, so "Kiểm tra ngay" gives no visible feedback on the real stack. `EchoResult` is discarded by the UI. | `internal/isonet/supervisor.go:102` `TriggerEcho` never calls `RecordEcho`; plan Ruling 11; R5.3 "Still open" | GW (+ WEB) | Call `RecordEcho` on success and emit `link.status`; optionally toast the `EchoResult` |
| NET-G2 | MCN-204-AC2 says WS emits `link.status` and `network.event` "on every change". Echo results and manual sign-on/off emit nothing. docs/04 §5 lists "echo result" as a `link.status` trigger. | `internal/api/network.go` handlers; `supervisor.go` only broadcasts from `setStatus`/`recordEvent` | GW | Broadcast after manual actions; record a `network_event` for each |
| NET-G3 | Sign-off leaves `link_state.status = SIGNED_ON` while the issuer holds the acquirer signed off. The UI shows healthy until a request returns RC 91. Sign-on doesn't write status either. | `supervisor.go:131` `TriggerSignOff` → `signOff` (no `setStatus`); R5.2 item 12 | GW | Set `DISCONNECTED` after 0800/002 (and `SIGNED_ON` after a manual sign-on), with events, or drop the manual sign-off endpoint |
| NET-G4 | WS payloads aren't the stored resources. `link.status` carries only endpoint and status (nulls elsewhere); `network.event` has `id: "0"` and a zero `occurredAt`. The page is unaffected (it only invalidates), but any consumer that reads `data` gets wrong values. | `supervisor.go:193`, `:201` | GW | Broadcast the row read back from the store (`RETURNING id, occurred_at`) |
...
| NET-G7 | `GET /v1/network/saf` is unbounded, and `depth` includes DEAD rows. The contract doesn't say whether it should. | `internal/store/safqueue.go:201` | GW + contracts | Cap items (e.g. 200) and define `depth` as PENDING + IN_FLIGHT in the contract |
| NET-G8 | The WebSocket bypasses the BFF: it relies on the uncommitted `NEXT_PUBLIC_WS_URL`, and the same-origin fallback `/v1/stream` isn't served by Next. | `src/shared/ws/useWsEvents.ts`; `web-next/.env.local` | WEB | Add a WS proxy in the BFF, or document the env var in `.env.example` |
| NET-G9 | Manual echo and sign-on wait up to the echo timeout (60 s = echo interval), and sign-off has no timeout at all. Both exceed the HTTP `WriteTimeout` (35 s), so the caller can get a dropped connection while the trigger lock still blocks the periodic echo. | `cmd/gateway/main.go:73` (no `EchoTimeout`), `:123`; `supervisor.go:82` | GW | Set `EchoTimeout` to ≤ 10 s (docs/03 §9 advice ack timeout order) |
| NET-G10 | A failed `ISSUER_DOWN` PUT shows nothing on this page. The Chaos Lab shows "Không đổi được sự cố. Hãy thử lại." | `NetworkScreen.tsx` `IssuerDownToggle` | WEB | Show the same one-line alert |
| NET-G11 | No card distinguishes "failed" from "empty": SAF shows "Trống" and links show "Chưa có đường kết nối nào…" when the gateway is down. | `SafCard.tsx`, `LinksPanel.tsx` | WEB | Render an error state from `query.isError` |
| NET-G12 | `GET /v1/network/events` ignores `limit` and `cursor`; `nextCursor` is always null. | `internal/api/network.go` (`defaultEventsLimit`); `?limit=2` returned 16 items | GW | Implement keyset pagination per docs/04 §2 |
| NET-G13 | `GET /v1/terminals` isn't implemented anywhere (404), though docs/04 routes it to gateway-go. | curl → 404; no handler in `internal/api` | GW | New story: terminal list from the acquirer `terminal` table |
| NET-G14 | Idempotency: the link actions ignore `Idempotency-Key`. The contract marks it required, and docs/04 §2 requires replay for 24 h. | `internal/api/network.go` | GW | Validate presence; replay is optional for echo |
