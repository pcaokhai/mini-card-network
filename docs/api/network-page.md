...
| NET-G5 | ~~The late-response event is broadcast but never persisted, so it isn't in `GET /v1/network/events` and vanishes on the refetch the WS event triggers.~~ | **Fixed** (#PR2): `purchase.Service.RecordLateResponse` persists the event before broadcasting it | GW | Done |
...
| 1.6 | 2026-09-26 | NET-G5 fixed (#PR2) |
