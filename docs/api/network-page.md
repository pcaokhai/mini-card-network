...
| NET-G17 | ~~Problem `type` values are bare slugs (`unknown-link`, `link-not-ready`), not docs/04 URIs, and `link-not-ready` isn't in the docs/04 §3 catalogue (the nearest is `link-down` 503).~~ | `internal/api/lab.go` `problem()`; `network.go:162` | GW | **Fixed** (#131, P-1): the shared URI problem writer; every slug is from the catalogue |
...
| 1.9 | 2026-09-26 | NET-G17 fixed (#131, P-1) |
